"""
    ...
    is able to load over 100 different types of documents and data, which is the first step of prepparing the
...
loader = TextLoader("resources/simple_text.txt")
doc = loader.load()
print(doc)
...
"""
    DirectoryLoader has a convinience argument called "show_progess" that will show a progress bar
...
def load_csv_files_only_from_directory(path: str):
    ...
    #       returned after loading
    loader = DirectoryLoader(path, show_progress=True, glob="**/*.csv", loader_cls=CSVLoader)
    return loader.load()